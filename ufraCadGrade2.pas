...
uses
  ...
  uniGUIClasses, uniGUIFrame, Data.DB, UniFSToggle, uniPanel, uniEdit,
  ...
  uniLabel, uniGUIBaseClasses, Datasnap.DBClient, UniSFiGrowl, UniSFSweetAlert;

type
  TfraCadGrade2 = class(TUniFrame)
...
uses uConstantes, uUtils, MainModule, RESTRequest4D.Request, System.JSON;

procedure TfraCadGrade2.carregaDados;
var
  resp1         :IResponse;
...
procedure TfraCadGrade2.retornar;
begin
  pgcTela.ActivePageIndex := 0;
  listar;
end;

procedure TfraCadGrade2.listar(pDado :string = '');
var
  resp1     :IResponse;
  jsonResp  :TJSONObject;
  ehNumero  :Boolean;
...
procedure TfraCadGrade2.limpaCampos;
begin
  alterando := false;
...
procedure TfraCadGrade2.btnCancelarClick(Sender: TObject);
begin
  limpaCookiesEditRecursivo(self);
  DefinirTodasAbasNaPrimeiraPagina(self);
end;

procedure TfraCadGrade2.btnNovoClick(Sender: TObject);
begin
  limpaCampos;
...
procedure TfraCadGrade2.btnSalvarClick(Sender: TObject);
var
  resp1       :IResponse;
...
procedure TfraCadGrade2.CDSTelabotaoEditarGetText(Sender: TField;
...
procedure TfraCadGrade2.CDSTelaiconeAtivoGetText(Sender: TField;
  var Text: string; DisplayText: Boolean);
begin
  ExibeIconeAtivo(CDSTela.FieldByName('ativo').AsString, Text, DisplayText);
end;

procedure TfraCadGrade2.compativoToggled(const Value: Boolean);
begin
  if value then
    ativo := 'S' else
    ativo := 'N';
...
procedure TfraCadGrade2.edPesquisarKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
    listar(edPesquisar.Text);
end;

procedure TfraCadGrade2.gridTelaCellClick(Column: TUniDBGridColumn);
begin
...
procedure TfraCadGrade2.gridTelaDrawColumnCell(Sender: TObject; ACol,
  ARow: Integer; Column: TUniDBGridColumn; Attribs: TUniCellAttribs);
begin
  SetGridColor(Sender, Attribs);
end;

procedure TfraCadGrade2.UniFrameCreate(Sender: TObject);
begin
  DefinirTodasAbasNaPrimeiraPagina(self);
...
procedure TfraCadGrade2.UniFrameReady(Sender: TObject);
begin
  limpaCookiesEditRecursivo(self);
